...
查五件事：
  ...
  6. 结尾有「讲的时候不要做的三件事」（§7.4 的结构第 5 条）
  7. 文件名是 `讲稿.md`（§7.4 的位置与名字）
...
def main(argv):
    ...
    # ⑥ 结尾三件事（§7.4 结构第 5 条）
    if re.search(r"^##[^\n]*讲的时候不要做的三件事", text, re.M):
        print(f"{OK} 结尾有「讲的时候不要做的三件事」")
    else:
        print(f"{BAD} 结尾缺「讲的时候不要做的三件事」—— 那三条是全场最容易犯的错"
              f"（念页面上的字 / 在重点页赶时间 / 说术语），写在最后一段")
        bad += 1

    # ⑦ 文件名（§7.4）：固定叫 讲稿.md，换名字下次就找不到
    if os.path.basename(script) == "讲稿.md":
        print(f"{OK} 文件名：讲稿.md")
    else:
        print(f"{BAD} 文件名是「{os.path.basename(script)}」—— 规范固定叫 讲稿.md"
              f"（放在 deck 同目录），换名字下次就找不到")
        bad += 1
