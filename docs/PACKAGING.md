...
agent **看不见**这个 skill，只能你打 `/skill:slides` 唤起。

**理由**：这套规范的前提很窄（**屏幕共享 · 对方窗口 ≥800px · 中英混排 · 芯原品牌色**）。前提之外它**不是中性的，它会主动害人** —— 拿去做投影 deck，`u` 的推导整个是错的。
...
- [x] **5.1** 装到 `~/.pi/agent/skills/slides/`
- [x] **5.2** **冷启动第三次**：全新会话 → `/skill:slides` + 一个题目 → 产出过 `check.py`
...
1. **冷启动第三次**：全新会话 → `/skill:slides` → 一个题目 → **产出的 deck 过 `check.py`，且你愿意分享**
...
| | 决定 | 理由 |
|---|---|---|
| **装哪** | **全局** `~/.pi/agent/skills/slides/` | 所有项目都能用 |
